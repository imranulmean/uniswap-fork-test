...
contract LiquidityMigrator {
	...
	constructor(address _router, address _pair, address _routerFork, _pairFork, address _bonusToken) public{
	...
	function migrate() external{
		require(msg.sender==admin, 'only admin can do this migration');
		require(migrationDone==false, 'migration already done');
		IERC20 token0= IERC20(pair.token0());
		IERC20 token1= IERC20(pair.token1());
		uint totalBalance= pair.balanceOf(address(this));

		router.removeLiquidity(address(token0), address(token1), totalBalance,0,0, address(this),
			block.timestamp);
		...
		routerFork.addLiquidity(address(token0), address(token1), token0Balance, token1Balance,
			token0Balance, token1Balance, address(this), block.timestamp);
		migrationDone=true;
	}
